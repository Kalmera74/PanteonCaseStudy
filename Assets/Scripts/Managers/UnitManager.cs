...
namespace Scripts.Managers
{
    ...
    public class UnitManager : MonoBehaviour
    {
        ...
        public void SpawnBuilding(BaseTile spawnPoint, int buildingIndex)
        {
            ...
            building.SetPosition(spawnPoint);
            spawnPoint.SetOccupier(building);
            OccupiedTiles.Add(spawnPoint);

            var soldierSpawnPos = spawnPoint.GetPosition();
            soldierSpawnPos.y -= .32f * 2f;
            var soldierSpawnTile = GridManager.Instance.GetTile(soldierSpawnPos);
            building.SetSpawnPoint(soldierSpawnTile);
